...
func resourceArmAutomationCredential() *schema.Resource {
	return &schema.Resource{
		...
		Importer: azSchema.ValidateResourceIDPriorToImport(func(id string) error {
			_, err := parse.AutomationCredentialID(id)
			return err
		}),
...
func resourceArmAutomationCredentialRead(d *schema.ResourceData, meta interface{}) error {
	...
	id, err := parse.AutomationCredentialID(d.Id())
	if err != nil {
		return err
	}
	...
	if err != nil {
		...
		return fmt.Errorf("Error making Read request on AzureRM Automation Credential '%s': %+v", id.Name, err)
	}
...
func resourceArmAutomationCredentialDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		...
		return fmt.Errorf("Error issuing AzureRM delete request for Automation Credential '%s': %+v", id.Name, err)
	}
