...
func resourceArmAutomationDscNodeConfiguration() *schema.Resource {
	return &schema.Resource{
		...
		Importer: azSchema.ValidateResourceIDPriorToImport(func(id string) error {
			_, err := parse.AutomationDscNodeConfigurationID(id)
			return err
		}),
...
func resourceArmAutomationDscNodeConfigurationRead(d *schema.ResourceData, meta interface{}) error {
	...
	id, err := parse.AutomationDscNodeConfigurationID(d.Id())
	if err != nil {
		return err
	}
	...
	if err != nil {
		...
		return fmt.Errorf("Error making Read request on AzureRM Automation Dsc Node Configuration %q: %+v", id.Name, err)
	}
...
func resourceArmAutomationDscNodeConfigurationDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		...
		return fmt.Errorf("Error issuing AzureRM delete request for Automation Dsc Node Configuration %q: %+v", id.Name, err)
	}
