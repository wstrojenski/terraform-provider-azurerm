...
func resourceArmAutomationModule() *schema.Resource {
	return &schema.Resource{
		...
		Importer: azSchema.ValidateResourceIDPriorToImport(func(id string) error {
			_, err := parse.AutomationModuleID(id)
			return err
		}),
...
func resourceArmAutomationModuleRead(d *schema.ResourceData, meta interface{}) error {
	...
	id, err := parse.AutomationModuleID(d.Id())
	if err != nil {
		return err
	}
	...
	if err != nil {
		...
		return fmt.Errorf("Error making Read request on AzureRM Automation Module %q: %+v", id.Name, err)
	}
...
func resourceArmAutomationModuleDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		...
		return fmt.Errorf("Error issuing AzureRM delete request for Automation Module %q: %+v", id.Name, err)
	}
